...
function App() {
  ...
  return (
    <>
      <Navbar />
      <div className="container  mx-auto my-5 rounded-xl p-5 bg-violet-500 min-h-[80vh] w-1/2">
        <h1 className="font-bold text-center text-xl">iTask Manage Your To-Do List</h1>
        <div className="addTodo">
          <h2 className="text-lg font-bold">Add ToDo</h2>
          <input
            onChange={handleChange} value={todo} type="text" className="w-1/2" />
          <button
        ...
        <input onChange={toggleFinished} type="checkbox" checked={showfinshed} />Show Finshed
        <h2 className="text-lg font-bold">Your Todos</h2>
        <div className="todos">
          {todos.length === 0 && (
            <span className="mx-3 ">No To-Do To Display</span>
          )}
          {todos.map((item) => {
